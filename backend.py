...
import spotipyimport os, re, uuid, logging, threading
from pathlib import Path
from flask import Flask, request, jsonify, send_from_directory
from flask_cors import CORS
import spotipy
...
app = Flask(__name__)
CORS(app) # Fixes the 'flask_cors' error once you add requirements.txt
jobs = {}

# ── SERVING YOUR UI (Fixes "Not Found") ────────────────────────────────────────

@app.route('/')
def serve_ui():
    # If your file is named 'now_playing_ui.html', rename it to 'localvibes.html'
...
def get_tracks(url):
    """Fetches track metadata. Spotify playlists MUST be Public."""
    if "spotify.com" in url:
        if "track" in url:
            sid = re.search(r"track/([A-Za-z0-9]+)", url).group(1)
            t = sp.track(sid)
            return t["name"], [{"title": t["name"], "artist": t["artists"][0]["name"], "album": t["album"]["name"], "art_url": t["album"]["images"][0]["url"]}]
        elif "playlist" in url:
            # NOTE: Server cannot see private playlists. Make it PUBLIC in Spotify.
    ...
    # YouTube Logic using your cookies.txt
    ...
    for i, track in enumerate(tracks):
        try:
            ydl_opts = {
                ...
                'cookiefile': COOKIE_FILE, # Bypasses YouTube bot/login check
